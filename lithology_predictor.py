...
<style>
  ...
  h1, h2 { color: #ffffff; }
  h2 { font-size: 15px !important; border-bottom: 1px solid #3a7bd5; padding-bottom: 6px; }
...
with st.sidebar:
    ...
    st.markdown('<div style="font-family:IBM Plex Mono;font-size:11px;color:#cce0ff">Enter log readings:</div>',
                unsafe_allow_html=True)
    ...
    if st.button("🔮 Predict Lithology", type="primary"):
        ...
        st.markdown(
            ...
            f'<div style="font-family:IBM Plex Mono;font-size:11px;color:#cce0ff">PREDICTED</div>'
        ...
        for lith, prob in top3:
            ...
            st.markdown(
                ...
                f'<span style="font-family:IBM Plex Mono;font-size:12px;color:#e8f0ff">{prob:.1%}</span>'
...
fig_logs.update_layout(
    **chart_layout(
        ...
        showlegend=True,
        hovermode="y unified",
        legend=dict(x=1.02, y=1, bgcolor="#0057C8", bordercolor="#3a7bd5", borderwidth=1)
    )
)
...
with c1:
    ...
    fig_acc.update_layout(**chart_layout(
        height=320,
        xaxis=dict(range=[0, 1.15], title="F1 Score", gridcolor="#3a7bd5"),
...
with cp1:
    ...
    fig_cp1.update_layout(**chart_layout(
        height=320,
        xaxis=dict(title="GR (API)", gridcolor="#3a7bd5"),
        yaxis=dict(title="RHOB (g/cc)", gridcolor="#3a7bd5"),
...
with cp2:
    ...
    fig_cp2.update_layout(**chart_layout(
        height=320,
        xaxis=dict(title="NPHI (v/v)", gridcolor="#3a7bd5"),
        yaxis=dict(title="RHOB (g/cc)", gridcolor="#3a7bd5"),
        margin=dict(l=60, r=20, t=20, b=50)
    ))
